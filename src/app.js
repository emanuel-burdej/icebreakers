...
const PHILOSOPHERS_STONE_TOPICS = [
    {
        key: 'flags',
        titleKey: 'topicFlags',
        desc: {
            sk: 'Hádajte krajinu podľa jej vlajky.',
            cz: 'Hádejte zemi podle její vlajky.',
            en: 'Guess the country by its flag.'
        }
    },
    {
        key: 'proverbs',
        titleKey: 'topicProverbs',
        desc: {
            sk: 'Doplňte chýbajúce slovo v slávnych výrokoch.',
...
function scaleIcon(iconHtml, size) {
    if (!iconHtml) return '';
    return iconHtml.replace(/width="22"/g, `width="${size}"`).replace(/height="22"/g, `height="${size}"`);
}

function getPhilosophersStoneTopicLabel(topicKey, lang = state.lang) {
    const i18nKey = TOPIC_I18N_KEY[topicKey];
    return (i18nKey && I18N[lang][i18nKey]) || topicKey || '';
}
...
function buildPhilosophersStoneModeHtml(size = 14) {
    return PHILOSOPHERS_STONE_TOPICS.map((topic) => {
        const icon = getPhilosophersStoneTopicIcon(topic.key, size);
        const label = I18N[state.lang][topic.titleKey];
        return `<span style="display:inline-flex; align-items:center; gap:4px; vertical-align:middle;">${icon} ${label}</span>`;
    }).join('<span style="opacity:0.2; margin:0 6px;">|</span>');
}

function resolvePackName(packKey, lang = state.lang) {
    const packI18nKey = PACK_I18N_KEY[packKey];
    return (packI18nKey && I18N[lang][packI18nKey]) || packKey || '...';
}
...
function getSessionMeta(data) {
    ...
    const isPhilosophersStone = data.packKey === 'philosophersStone' || data.mode === 'philosophersStone_play';
    ...
    const rawPackKey = data.packKey || 'basic';
    ...
    return {
        ...
        packName: resolvePackName(rawPackKey),
        modeName,
        modeIcon
    };
}
...
function renderModeSelection() {
    ...
    if (state.packKey === 'philosophersStone') {
        PHILOSOPHERS_STONE_TOPICS.forEach((topic) => {
            const name = I18N[state.lang][topic.titleKey];
            const desc = topic.desc[state.lang] || topic.desc.en;
            const icon = getPhilosophersStoneTopicIcon(topic.key, 28);
            const div = document.createElement('div');
            div.className = 'glass';
            div.style = 'padding: 20px; margin-bottom: 12px; cursor: pointer; display: flex; align-items: center; gap: 18px;';
            div.onclick = () => selectPhilosophersStoneTopic(topic.key);
            div.innerHTML = `
...
function updateStats() {
    ...
    let modeName = "";
    if (state.packKey === 'philosophersStone') {
        modeName = getPhilosophersStoneTopicLabel(state.selectedTopic);
    } else {
...
function renderResult() {
    ...
    if (isLogoQuizCard(card)) {
        ...
        if (isFlags) {
            customHtml = `
                <div class="ps-flags-layout">
                    <div class="ps-flag-display">${card.flag}</div>
                    <div class="masked-word-container ps-flags">${lettersHtml}</div>
                    ${getPsCapitalHtml(card)}
                </div>
            `;
        } else {
            ...
            customHtml = `
                <div class="proverb-display-container" style="text-align: center; margin: 20px 0; font-family: 'Plus Jakarta Sans', sans-serif;">
                    ...
                    ${(card.isTranslationRevealed || card.isFullRevealed) ? `<p class="proverb-translation" style="font-style: italic; opacity: 0.8; font-size: 1.1rem; margin-top: 25px; transition: opacity 0.5s;">„${card.translations[state.lang] || card.translations.en}“</p>` : ''}
                </div>
            `;
        }
    ...
    const translationBtn = document.getElementById('ps-translation-btn');
    if (translationBtn) {
        const showTranslationBtn = card.cardType === 'philosophersStone' && card.topic === 'proverbs';
        translationBtn.classList.toggle('is-hidden-display', !showTranslationBtn);
        const translationRevealed = card.isTranslationRevealed || card.isFullRevealed;
        translationBtn.disabled = translationRevealed;
        translationBtn.title = I18N[state.lang].revealTranslation;
        translationBtn.setAttribute('aria-label', I18N[state.lang].revealTranslation);
    }
    const capitalBtn = document.getElementById('ps-capital-btn');
    if (capitalBtn) {
        const showCapitalBtn = card.cardType === 'philosophersStone' && card.topic === 'flags' && card.capital;
        capitalBtn.classList.toggle('is-hidden-display', !showCapitalBtn);
        const capitalRevealed = card.isCapitalRevealed || card.isFullRevealed;
        capitalBtn.disabled = capitalRevealed;
        capitalBtn.title = I18N[state.lang].revealCapital;
...
function buildPsMaskedLettersHtml(card, word, compact = false) {
    if (!card.revealedIndices) {
        card.revealedIndices = [];
    }
    return word.split('').map((char, idx) => {
        if (char === ' ' || char === '-') {
            if (compact) {
                return `<span class="ps-flag-separator">${char === '-' ? '‑' : ''}</span>`;
            }
        ...
        const letterClass = compact ? 'masked-letter ps-flag-letter' : 'masked-letter';
        return `<span class="${letterClass} ${isRevealed ? '' : 'hidden'}">${char}</span>`;
    }).join('');
}

function getPsCapitalHtml(card) {
    if (!card.capital || !(card.isCapitalRevealed || card.isFullRevealed)) return '';
    const capital = card.capital[state.lang] || card.capital.en;
    return `<p class="ps-capital">${escapeHtml(I18N[state.lang].capitalLabel)} ${escapeHtml(capital)}</p>`;
}
...
function revealProverbTranslation() {
    const card = state.history[state.historyIndex];
    if (!card || card.cardType !== 'philosophersStone' || card.topic !== 'proverbs') return;
    if (card.isTranslationRevealed || card.isFullRevealed) return;

    card.isTranslationRevealed = true;
    playTick();
    saveCurrentState();
    renderResult();
}

function revealFlagCapital() {
    const card = state.history[state.historyIndex];
    if (!card || card.cardType !== 'philosophersStone' || card.topic !== 'flags') return;
    if (card.isCapitalRevealed || card.isFullRevealed) return;

    card.isCapitalRevealed = true;
    playTick();
    saveCurrentState();
    renderResult();
}
